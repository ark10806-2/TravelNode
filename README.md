...
숙소 주변 여행 장소를 Postgres에 저장하고, Ktor API로 불러와 React + shadcn 화면의 Google Map 위에 표시하는 간단한 풀스택 앱입니다.
...
카테고리/장소 추가, 설명 수정, 삭제, Google Maps 링크 미리보기/즐겨찾기 목록 동기화처럼 데이터를 바꾸거나 외부 API를 호출하는 요청은 로그인 후 받은 `Authorization: Bearer <token>` 헤더가 필요합니다.
