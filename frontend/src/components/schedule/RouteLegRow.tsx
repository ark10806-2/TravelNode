...
export function RouteLegRow({
  ...
  return (
    <div className="grid grid-cols-[1.75rem_minmax(0,1fr)] px-1.5 sm:grid-cols-[2rem_minmax(0,1fr)] sm:px-2.5">
      <div className="relative flex min-h-16 justify-center sm:min-h-12">
        <div className="absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-border/60" />
        {isEditing && onToggleLock ? (
          <button
            type="button"
            className={cn(
              'absolute left-1/2 top-1/2 z-10 grid h-6 w-6 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border bg-background text-muted-foreground shadow-sm transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
          ...
          <span
            className="absolute left-1/2 top-1/2 z-10 grid h-6 w-6 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-primary/25 bg-primary/10 text-primary"
            aria-label="고정된 구간"
            title="고정된 장소 사이 구간"
          >
      ...
      <div className="relative flex min-h-16 min-w-0 items-center py-1 sm:min-h-12">
        <div className="absolute inset-x-0 top-1/2 h-px bg-border/45" />
        <div
          className="relative w-full max-w-[22rem] rounded-xl bg-background/95 px-2 py-1.5 text-[11px] text-muted-foreground/80 shadow-sm shadow-black/5 sm:mr-auto sm:max-w-[34rem] sm:px-1.5 sm:py-1"
        >
          {departureTimeMinutes != null ? (
            <div className="mb-1 flex flex-wrap items-center gap-1 sm:absolute sm:left-1.5 sm:top-[-0.65rem] sm:mb-0">
              <span className="rounded-full bg-secondary px-1.5 py-0.5 text-[10px] font-semibold leading-none text-muted-foreground/75 sm:text-[9px]">
                {formatDepartureTime(departureTimeMinutes)}
              </span>
            </div>
          ) : null}
          <div
            className="grid items-stretch gap-1 sm:items-center sm:gap-1.5"
            ...
            {modes.map((mode) => {
              ...
              return (
                <a
                  key={mode}
                  className={cn(
                    'min-w-0 content-center items-center justify-self-stretch rounded-md px-1.5 py-1 text-left underline-offset-4 transition hover:bg-muted/50 hover:text-foreground hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex sm:gap-1 sm:px-1 sm:py-0.5',
                    keepModeLine
                ...
                >
                  {isLoading ? (
                    <Loader2
                      className={cn(
                        'h-3.5 w-3.5 animate-spin text-muted-foreground/55',
                  ...
                  {!isLoading && !isPending ? (
                    <span
                      className={cn(
                        'w-fit max-w-full break-keep rounded-full px-1 py-0.5 text-[9px] font-medium leading-none sm:text-[10px]',
                        keepModeLine
